...
void Font::make_dlist( FT_Face face, char ch, float colour[3]) {
	...
	col[12] = colour[0]; col[13] = colour[1]; col[14] = colour[2];
	col[15] = colour[0]; col[16] = colour[1]; col[17] = colour[2];
...
void Font::init(const char * fname, unsigned int h, float colour[3]) {
...
///with freetype fonts.
void print(glm::mat4 &ProjectionMatrix, Font &ft_font, float x, float y, const char *fmt, ...)  {
	...
	glUniformMatrix4fv(glGetUniformLocation(Font::myShader->handle(), "ProjectionMatrix"), 1, GL_FALSE, &ProjectionMatrix[0][0]);

	glUniform1i(glGetUniformLocation(Font::myShader->handle(), "DiffuseMap"), 0);

	for(int i=0;i<lines.size();i++) {
		
		//float t[16];
		//MatrixRoutines<float>::setTranslation(x,y-h*i, 0.0, t);
		glm::mat4 t = glm::translate(glm::mat4(1.0), glm::vec3(x, y - h*i, 0.0));
		...
		for(int k=0; k<lines[i].length(); k++)
		{
			...
			glUniformMatrix4fv(glGetUniformLocation(Font::myShader->handle(), "ModelViewMatrix"), 1, GL_FALSE, &t[0][0]);
			...
			t = glm::translate(t, glm::vec3(ft_font.widthChar[c[k]], 0.0, 0.0));
		}
